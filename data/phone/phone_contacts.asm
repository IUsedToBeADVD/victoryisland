MACRO phone
	db  \1, \2 ; trainer
	map_id \3 ; map
	db  \4
	dba \5 ; script 1
	db  \6
	dba \7 ; script 2
ENDM

PhoneContacts:
; entries correspond to PHONE_* constants
	table_width PHONE_CONTACT_SIZE
	phone TRAINER_NONE, PHONE_00,              N_A,                       %0000, UnusedPhoneScript,   %0000, UnusedPhoneScript
	phone TRAINER_NONE, PHONECONTACT_MOM,      PLAYERS_HOUSE_1F,          %1111, MomPhoneScript,      %0000, UnusedPhoneScript
	phone TRAINER_NONE, PHONECONTACT_BIKESHOP, N_A,                  %0000, UnusedPhoneScript,   %0000, UnusedPhoneScript
	phone TRAINER_NONE, PHONECONTACT_BILL,     N_A,                       %1111, BillPhoneScript1,    %0000, BillPhoneScript2
	phone TRAINER_NONE, PHONECONTACT_ELM,      N_A,                  %1111, ElmPhoneScript1,     %0000, ElmPhoneScript2
	phone TRAINER_NONE, PHONECONTACT_LYRA,     N_A,            %1111, LyraPhoneScript,     %0000, LyraPhoneScript2
	phone SCHOOLBOY,    JACK1,                 N_A,             %1111, JackPhoneScript1,    %1111, JackPhoneScript2
	phone POKEFANF,     BEVERLY1,              N_A,             %1111, BeverlyPhoneScript1, %1111, BeverlyPhoneScript2
	phone SAILOR,       HUEY1,                 N_A,     %1111, HueyPhoneScript1,    %1111, HueyPhoneScript2
	phone TRAINER_NONE, PHONE_00,              N_A,                       %0000, UnusedPhoneScript,   %0000, UnusedPhoneScript
	phone TRAINER_NONE, PHONE_00,              N_A,                       %0000, UnusedPhoneScript,   %0000, UnusedPhoneScript
	phone COOLTRAINERM, GAVEN1,                N_A,                  %1111, GavenPhoneScript1,   %1111, GavenPhoneScript2
	phone COOLTRAINERF, BETH1,                 N_A,                  %1111, BethPhoneScript1,    %1111, BethPhoneScript2
	phone BIRD_KEEPER,  JOSE1,                 N_A,                  %1111, JosePhoneScript1,    %1111, JosePhoneScript2
	phone COOLTRAINERF, REENA1,                N_A,                  %1111, ReenaPhoneScript1,   %1111, ReenaPhoneScript2
	phone YOUNGSTER,    JOEY1,                 N_A,                  %1111, JoeyPhoneScript1,    %1111, JoeyPhoneScript2
	phone BUG_CATCHER,  WADE1,                 N_A,                  %1111, WadePhoneScript1,    %1111, WadePhoneScript2
	phone FISHER,       RALPH1,                N_A,                  %1111, RalphPhoneScript1,   %1111, RalphPhoneScript2
	phone PICNICKER,    LIZ1,                  N_A,                  %1111, LizPhoneScript1,     %1111, LizPhoneScript2
	phone HIKER,        ANTHONY1,              N_A,                  %1111, AnthonyPhoneScript1, %1111, AnthonyPhoneScript2
	phone CAMPER,       TODD1,                 N_A,                  %1111, ToddPhoneScript1,    %1111, ToddPhoneScript2
	phone PICNICKER,    GINA1,                 N_A,                  %1111, GinaPhoneScript1,    %1111, GinaPhoneScript2
	phone JUGGLER,      IRWIN1,                N_A,                  %1111, IrwinPhoneScript1,   %1111, IrwinPhoneScript2
	phone BUG_CATCHER,  ARNIE1,                N_A,                  %1111, ArniePhoneScript1,   %1111, ArniePhoneScript2
	phone SCHOOLBOY,    ALAN1,                 N_A,                  %1111, AlanPhoneScript1,    %1111, AlanPhoneScript2
	phone TRAINER_NONE, PHONE_00,              N_A,                       %0000, UnusedPhoneScript,   %0000, UnusedPhoneScript
	phone LASS,         DANA1,                 N_A,                  %1111, DanaPhoneScript1,    %1111, DanaPhoneScript2
	phone SCHOOLBOY,    CHAD1,                 N_A,                  %1111, ChadPhoneScript1,    %1111, ChadPhoneScript2
	phone POKEFANM,     DEREK1,                N_A,                  %1111, DerekPhoneScript1,   %1111, DerekPhoneScript2
	phone FISHER,       TULLY1,                N_A,                  %1111, TullyPhoneScript1,   %1111, TullyPhoneScript2
	phone POKEMANIAC,   BRENT1,                N_A,                  %1111, BrentPhoneScript1,   %1111, BrentPhoneScript2
	phone PICNICKER,    TIFFANY1,              N_A,                  %1111, TiffanyPhoneScript1, %1111, TiffanyPhoneScript2
	phone BIRD_KEEPER,  VANCE1,                N_A,                  %1111, VancePhoneScript1,   %1111, VancePhoneScript2
	phone FISHER,       WILTON1,               N_A,                  %1111, WiltonPhoneScript1,  %1111, WiltonPhoneScript2
	phone BLACKBELT_T,  KENJI1,                N_A,                  %1111, KenjiPhoneScript1,   %1111, KenjiPhoneScript2
	phone HIKER,        PARRY1,                N_A,                  %1111, ParryPhoneScript1,   %1111, ParryPhoneScript2
	phone PICNICKER,    ERIN1,                 N_A,                  %1111, ErinPhoneScript1,    %1111, ErinPhoneScript2
	phone TRAINER_NONE, PHONECONTACT_BUENA,    N_A, %1111, BuenaPhoneScript1,   %1111, BuenaPhoneScript2
	assert_table_length NUM_PHONE_CONTACTS + 1
